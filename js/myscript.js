...
jQuery(document).ready(function() {
    ...
    function checkFormFill() {
        return $('#name').val() != '' && $('#email').val() != '' && $('#address').val() != '' && $('#phone').val() != '';
    }

    function calculateProgressBar(total_amount) {
        var progress = 25;
        if (total_amount > 0)
            progress += 25;
        if (checkFormFill())
            progress += 25;
        if (progress >= 75) {
            $('#submit-res').removeClass('disabled');
            $('#submit-res').removeAttr("disabled", "disabled");
        } else {
            $('#submit-res').addClass('disabled');
	...
	function setProgressBar(progress) {
		...
		$('#progress-bar span').html(progress + ' % completed');
	}
	
	$('#submit-res').click(function(event) {
...
/*-----------------------------------------------------------------------------------*/
jQuery(document).ready(function() {
	$.getJSON("http://api.flickr.com/services/feeds/photos_public.gne?id=59247791@N08&lang=en-us&format=json&jsoncallback=?", function(data){
